...
// Copyright (c) 2013 The Quarkbar developers
...
uint256 hashGenesisBlock("0x0000074b7f9ebce823d6f7856694df606f02d8a350c4450eaa72ef80e41b85d2");
static const unsigned int timeGenesisBlock = 1393675200; //Sat, 01 Mar 2014 12:00:00 GMT
...
const string strMessageMagic = "Quarkbar Signed Message:\n";
...
static const int64 nBlockRewardStartCoin = 128 * COIN;
static const int64 nBlockRewardMinimumCoin = 2 * COIN;

static const int64 nTargetTimespan = 10 * 120; // 20 minutes
...
int64 static GetBlockValue(int nHeight, int64 nFees, unsigned int nBits)
{
    ...
    // Subsidy is cut in half every 86400 blocks (14 days)
    nSubsidy >>= (nHeight / 40320);
...
{
        static const int64 BlocksTargetSpacing = 30; // 2 minute blocks
        unsigned int TimeDaySeconds = 86400; //60 * 60 * 24;
        int64 PastSecondsMin = 864; //TimeDaySeconds * 0.01;
        int64 PastSecondsMax = 12096; //TimeDaySeconds * 0.14;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "Joe and Teresa Giudice to Plead Guilty for Fraud, Other Charges";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce   = 11657122;

        if (fTestNet)
        {
            block.nTime    = 1393675200;
            block.nNonce   = 0;
        }
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x526af5cc1e2d5e04b3dfe1bf2acffc22ffeec17b8cdb476383abf67ea85b9eeb"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("QuarkbarMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("QuarkbarMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("QuarkbarMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("quarkbar-miner");
    ...
    try { while (true) {
        ...
        printf("Running QuarkbarMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("QuarkbarMiner terminated\n");
        throw;
    }
}
